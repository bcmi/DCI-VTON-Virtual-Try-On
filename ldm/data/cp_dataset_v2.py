...
class CPDataset(data.Dataset):
    ...
    def get_agnostic(self, im, im_parse, pose_data):
        ...
        agnostic_mask = Image.new('L', (768, 1024), 'black')
        ...
        for i in [9, 12]:
            ...
            agnostic_mask_draw.ellipse((pointx - r * 3, pointy - r * 6, pointx + r * 3, pointy + r * 6), 'white',
                                       'white')
        agnostic_draw.line([tuple(pose_data[i]) for i in [2, 9]], 'gray', width=r * 6)
        agnostic_draw.line([tuple(pose_data[i]) for i in [5, 12]], 'gray', width=r * 6)
        agnostic_draw.line([tuple(pose_data[i]) for i in [9, 12]], 'gray', width=r * 12)
        agnostic_draw.polygon([tuple(pose_data[i]) for i in [2, 5, 12, 9]], 'gray', 'gray')
        agnostic_mask_draw.line([tuple(pose_data[i]) for i in [2, 9]], 'white', width=r * 6)
        agnostic_mask_draw.line([tuple(pose_data[i]) for i in [5, 12]], 'white', width=r * 6)
        agnostic_mask_draw.line([tuple(pose_data[i]) for i in [9, 12]], 'white', width=r * 12)
        agnostic_mask_draw.polygon([tuple(pose_data[i]) for i in [2, 5, 12, 9]], 'white', 'white')

        # mask neck
        pointx, pointy = pose_data[1]
        agnostic_draw.rectangle((pointx - r * 5, pointy - r * 9, pointx + r * 5, pointy), 'gray', 'gray')
        agnostic_mask_draw.rectangle((pointx - r * 5, pointy - r * 9, pointx + r * 5, pointy), 'white', 'white')

        # mask arms
        agnostic_draw.line([tuple(pose_data[i]) for i in [2, 5]], 'gray', width=r * 12)
        agnostic_mask_draw.line([tuple(pose_data[i]) for i in [2, 5]], 'white', width=r * 12)
        for i in [2, 5]:
            pointx, pointy = pose_data[i]
            agnostic_draw.ellipse((pointx - r * 5, pointy - r * 6, pointx + r * 5, pointy + r * 6), 'gray', 'gray')
            agnostic_mask_draw.ellipse((pointx - r * 5, pointy - r * 6, pointx + r * 5, pointy + r * 6), 'white',
                                       'white')
        for i in [3, 4, 6, 7]:
            ...
            agnostic_mask_draw.line([tuple(pose_data[j]) for j in [i - 1, i]], 'white', width=r * 10)
            pointx, pointy = pose_data[i]
            agnostic_draw.ellipse((pointx - r * 5, pointy - r * 5, pointx + r * 5, pointy + r * 5), 'gray', 'gray')
            agnostic_mask_draw.ellipse((pointx - r * 5, pointy - r * 5, pointx + r * 5, pointy + r * 5), 'white',
                                       'white')
        remain_mask = Image.new('L', (768, 1024), 'black')
    ...
    def __getitem__(self, index):
        ...
        agnostic, agnostic_mask = self.get_agnostic(im_pil_big, im_parse_pil_big, pose_data)
        agnostic = transforms.Resize(self.crop_size, interpolation=2)(agnostic)
        agnostic_mask = transforms.Resize(self.crop_size, interpolation=0)(agnostic_mask)
        agnostic = self.transform(agnostic)
        agnostic_mask = np.array(agnostic_mask)
        agnostic_mask = (agnostic_mask >= 128).astype(np.float32)
        agnostic_mask = torch.from_numpy(agnostic_mask).unsqueeze(0)

        inpaint_mask = 1 - agnostic_mask
        inpaint = agnostic + warped_cloth * (1 - inpaint_mask)
        feat = inpaint
...
if __name__ == '__main__':
    dataset = CPDataset('/mnt/workspace/VITON-HD', 512, mode='train', unpaired=False)
