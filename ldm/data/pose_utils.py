...
if __name__ == "__main__":
    ...
    i = 5
    df = pd.read_csv('../data/fasion-annotation-train.csv', sep=':')

    for index, row in df.iterrows():
        pose_cords = load_pose_cords_from_strings(row['keypoints_y'], row['keypoints_x'])
        pose_map = cords_to_map(pose_cords, (256, 176), (256, 176))
        colors, mask = draw_pose_from_cords(pose_cords, (256, 176))

        mmm = produce_ma_mask(pose_cords, (256, 176)).astype(float)[..., np.newaxis].repeat(3, axis=-1)
        print(mmm.shape)
